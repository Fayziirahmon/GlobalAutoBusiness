"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var express_1 = require("express");
var mongoose_1 = require("mongoose");
var cors_1 = require("cors");
var dotenv_1 = require("dotenv");
dotenv_1.default.config();
var app = (0, express_1.default)();
// Middleware'lar
app.use((0, cors_1.default)());
app.use(express_1.default.json());
var ProductSchema = new mongoose_1.default.Schema({
    title: { type: String, required: true },
    description: { type: String, required: true },
    price: { type: Number, required: true },
    imageUrl: { type: String, required: true }
});
var Product = mongoose_1.default.model('Product', ProductSchema);
// 2. Mahsulotlarni olish (pagination bilan — har sahifada 50 ta)
app.get('/api/products', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var page, limit, skip, q, filter, _a, products, totalProducts, error_1;
    var _b;
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0:
                _c.trys.push([0, 2, , 3]);
                page = Math.max(1, parseInt(req.query.page) || 1);
                limit = Math.min(100, Math.max(1, parseInt(req.query.limit) || 50));
                skip = (page - 1) * limit;
                q = (_b = req.query.q) === null || _b === void 0 ? void 0 : _b.trim();
                filter = q ? { title: { $regex: q, $options: 'i' } } : {};
                return [4 /*yield*/, Promise.all([
                        Product.find(filter).skip(skip).limit(limit),
                        Product.countDocuments(filter),
                    ])];
            case 1:
                _a = _c.sent(), products = _a[0], totalProducts = _a[1];
                res.status(200).json({
                    products: products,
                    currentPage: page,
                    totalPages: Math.max(1, Math.ceil(totalProducts / limit)),
                    totalProducts: totalProducts,
                    limit: limit,
                });
                return [3 /*break*/, 3];
            case 2:
                error_1 = _c.sent();
                res.status(500).json({ error: 'Ma\'lumotlarni olishda xatolik yuz berdi' });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); });
// 2b. Bitta mahsulotni ID bo'yicha olish (alohida sahifa uchun)
app.get('/api/products/:id', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var product, error_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, Product.findById(req.params.id)];
            case 1:
                product = _a.sent();
                if (!product) {
                    return [2 /*return*/, res.status(404).json({ error: 'Mahsulot topilmadi' })];
                }
                res.status(200).json(product);
                return [3 /*break*/, 3];
            case 2:
                error_2 = _a.sent();
                // Noto'g'ri ID formati (CastError) ham shu yerga tushadi
                res.status(400).json({ error: 'Mahsulot topilmadi yoki ID noto\'g\'ri' });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); });
// 3. Mahsulot qo'shish
app.post('/api/products', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, title, description, price, imageUrl, newProduct, error_3;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 2, , 3]);
                _a = req.body, title = _a.title, description = _a.description, price = _a.price, imageUrl = _a.imageUrl;
                // Validatsiya: hamma maydonlar to'ldirilganligini tekshirish
                if (!title || !price || !imageUrl) {
                    return [2 /*return*/, res.status(400).json({ error: 'Iltimos, barcha kerakli maydonlarni to\'ldiring' })];
                }
                newProduct = new Product({ title: title, description: description, price: price, imageUrl: imageUrl });
                return [4 /*yield*/, newProduct.save()];
            case 1:
                _b.sent();
                res.status(201).json(newProduct);
                return [3 /*break*/, 3];
            case 2:
                error_3 = _b.sent();
                res.status(500).json({ error: 'Mahsulotni saqlashda xatolik yuz berdi' });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); });
// 4. Serverni ishga tushirish va bazaga ulanish
var PORT = process.env.PORT || 5000;
var MONGO_URI = process.env.MONGO_URI;
mongoose_1.default.connect(MONGO_URI)
    .then(function () {
    console.log('MongoDB ga muvaffaqiyatli ulanildi');
    app.listen(PORT, function () { return console.log("Server ".concat(PORT, "-portda ishlayapti")); });
})
    .catch(function (err) { return console.error('MongoDB ulanish xatosi:', err); });
